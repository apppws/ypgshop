...
<body>
    <div class="margin clearfix">
        <div class="sort_style">
            <div class="border clearfix">
                <span class="l_f">
                    <a href="javascript:ovid()" id="add_page" class="btn btn-warning" onclick="add_article_sort()"><i
                ...
                <table class="table table-striped table-bordered table-hover" id="sample-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $v->id }}</td>
                            <td>{{str_repeat('-',8*$v->level). $v->cat_name }}</td>
                            <td>{{ $v->created_at }}</td>
                            <td class="td-manage">
                                <a title="编辑"  href="{{ route('blogcat_edit',['id'=>$v->id]) }}" class="btn btn-xs btn-info"><i
                                        class="fa fa-edit bigger-120"></i></a>
                                <a title="删除" href="{{ route('blogcat_delete',['id'=>$v->id]) }}" onclick="confirm('确定要删除我吗?(╥╯^╰╥)')" class="btn btn-xs btn-danger"><i
    ...
    <div id="addsort_style" style="display:none" class="article_style">
        <form action="{{ route('blogcat_doadd') }}" method="POST">
            @csrf
