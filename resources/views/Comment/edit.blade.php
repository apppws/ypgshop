...
<body>
    <div class=" clearfix">
    <form action="{{ route('comment_doedit',['id'=>$data->id,'goods_id'=>$data->goods_id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
        <div id="add_brand" class="clearfix">
            <div class="left_add">
                <div class="title_name">添加评论</div>
                <ul class="add_conent">
                    <li class=" clearfix"><label class="label_name"><i>*</i>评分：</label>
                        <input type="radio" name="star" value="1" @if($data->star==1) checked @endif> 1分
                        <input type="radio" name="star" value="2" @if($data->star==2) checked @endif> 2分
                        <input type="radio" name="star" value="3" @if($data->star==3) checked @endif> 3分
                        <input type="radio" name="star" value="4" @if($data->star==4) checked @endif> 4分
                        <input type="radio" name="star" value="5" @if($data->star==5) checked @endif> 5分
                    </li>
                    <li class=" clearfix"><label class="label_name">品牌图片：</label>
                        <!-- 预览图片 -->
                        <div class="add-article-box-content">
                            <div class="img-preview" style="display: block; width: 150px; height: 121px;z-index:10">
                                <img src="{{ '/upload/'.$data->logo }}" width="100px" alt="" height="100px" /></div>
                        </div>
                        <div class="demo l_f">
                            <div class="add-article-box-content">
                                <!-- 保存裁切时的区域信息 -->
                    ...
                    <li class=" clearfix"><label class="label_name">品牌描述：</label>
                        <textarea name="content" cols="" rows="" class="textarea" onkeyup="checkLength(this);">{{ $data->content }}</textarea>
                        <span class="wordage">品牌描述：
                            <span id="sy" style="color:Red;">500</span>字
                        </span>
                    </li>
                    <li class=" clearfix"><label class="label_name"><i>*</i>是否显示：</label>
                        <input type="radio" name="is_show" value="y" @if($data->is_show=='y') checked @endif> 显示
                    <input type="radio" name="is_show" value="n" @if($data->is_show=='n') checked @endif> 不显示
                    </li>
                </ul>
            </div>
