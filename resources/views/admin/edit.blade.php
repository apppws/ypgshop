...
<body>
        <div class="Competence_add_style clearfix">
                <form action="{{ route('admin_doedit',['id'=>$data->id]) }}" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
                    @csrf
                    <div class="left_Competence_add">
                   <div class="title_name">修改管理员</div>
                    <div class="Competence_add">
                        <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 账号 </label>
                            <div class="col-sm-9">
                                <input type="text" id="form-field-1" placeholder="" value="{{ $data->name }}"  name="name" class="col-xs-10 col-sm-5">
                            </div>
                        </div>
                        <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 密码 </label>
                            <div class="col-sm-9">
                                <input type="password" id="form-field-1" placeholder=""  name="password" class="col-xs-10 col-sm-5">
                            </div>
                        </div>
                        <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 确认密码 </label>
                            <div class="col-sm-9">
                                <input type="password" id="form-field-1" placeholder=""  name="cpassword" class="col-xs-10 col-sm-5">
                            </div>
                        </div>
                        <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 角色列表 </label>
                            <div class="col-sm-9">
                                    @if($data->id>1)
                                    <tr>
                                        <td>
                                        @foreach($roleData as $v)
                                        <input @if(in_array($v->id,$roleId)) checked="checked" @endif type="checkbox" name="role_id[]" value="{{$v->id}}"> {{$v->role_name}}
                                        @endforeach
                                        </td>
                                    </tr>
                                    @endif
                        ...
                        <div class="Button_operation">
                            ...
                            <button onclick="history.go(-1)" class="btn btn-secondary  btn-warning" type="button"><i class="fa fa-reply"></i> 返回上一步</button>
